...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if this is a generate request (not update)
    $isGenerateRequest = true; // Assume it's a generate request by default
    if (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'updateLogic.php') !== false) {
        $isGenerateRequest = false;
    }

    // Check for existing entries if this is a generate request
                ...
                <body>
                    <div class='container'>
                        <div class='error-message'>
                            <span style='font-size: 50px;'>❌</span>  <!-- Warning emoji -->
                            <p>Data already exists for <strong>{$subject_name}</strong> in semester <strong>{$sem_id}</strong>, division <strong>{$division}</strong>.</p>
                            <p>Please use the <strong>Update Dates</strong> button if you want to modify the existing data.</p>
                        </div>
    ...
    <body>
        <div class='container'>
            <div class='success-message'>
                <span style='font-size: 50px;'>✅</span>  <!-- Checkmark emoji -->
