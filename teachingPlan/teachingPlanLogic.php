...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $verifiedByHod = $_POST['verified_by_hod'] ?? [];
        ...
        foreach ($contents as $pk => $content) {
            ...
            $coMapping = $coMappings[$pk] ?? null;
            $verifiedByHodStatus = isset($verifiedByHod[$pk]) ? 1 : 0; // Checkbox returns 1 if checked

            // Prepare and execute the update query
            $updateSql = "UPDATE teaching_plan SET content = :content, actual_date = :actual_date, content_not_covered = :content_not_covered, reference = :reference, methodology = :methodology, co_mapping = :co_mapping, verified_by_hod = :verified_by_hod WHERE pk = :pk";
            ...
            $updateStmt->execute([
                ...
                'verified_by_hod' => $verifiedByHodStatus,
                'pk' => $pk
            ]);
        }
        ...
        <table>
            ...
            <?php foreach ($plans as $plan): ?>
                <tr>
                    ...
                    <td>
                        <!-- Render an empty text area instead of date input -->
                        <textarea 
                            class="editable-input" 
                            name="actual_date[<?= $plan['pk'] ?>]" 
                            placeholder="No date selected" 
                            <?= $editable == 0 ? 'readonly' : '' ?>
                            style="height: auto; resize: none;">
                            <?= htmlspecialchars($plan['actual_date'] ?: '') ?>
                        </textarea>
                    </td>
                    <td>
                        <textarea 
                    ...
                    <td>
                        <select name="co_mapping[<?= $plan['pk'] ?>]" class="editable-input" <?= $editable == 0 ? 'disabled' : '' ?>>
                            <option value="CO-1" <?= $plan['co_mapping'] == 'CO-1' ? 'selected' : '' ?>>CO-1</option>
                            <option value="CO-2" <?= $plan['co_mapping'] == 'CO-2' ? 'selected' : '' ?>>CO-2</option>
                            <option value="CO-3" <?= $plan['co_mapping'] == 'CO-3' ? 'selected' : '' ?>>CO-3</option>
                            <option value="CO-4" <?= $plan['co_mapping'] == 'CO-4' ? 'selected' : '' ?>>CO-4</option>
                            <option value="CO-5" <?= $plan['co_mapping'] == 'CO-5' ? 'selected' : '' ?>>CO-5</option>
                        </select>
                    </td>
                    <td>
                    <!-- Add any other field or buttons as required -->
                    </td>
                </tr>
            <?php endforeach; ?>
        </table>
